<?php $__env->startSection('title', 'Turni Macchine'); ?>
...
<?php if(auth()->check() && (auth()->user()->admin ?? 0) != 1): ?>
  <style>
    #logout-timer{
  ...
  <form id="auto-logout-form" action="<?php echo e(route('logout')); ?>" method="POST" class="d-none">
    <?php echo csrf_field(); ?>
  </form>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function load(){
    ...
    const url = `<?php echo e(route('assmac.json')); ?>?data=${encodeURIComponent($date.value)}`;
    try{
  ...
  document.getElementById('m-salva').addEventListener('click', async () => {
    ...
    try{
      await fetch(`<?php echo e(route('assmac.nota.save')); ?>`, {
        method:'POST',
...
<?php echo $__env->make('adminlte::page', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\guala\guala-app\resources\views/app/macchine/index.blade.php ENDPATH**/ ?>