...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch("<?php echo e(route('ordini.json')); ?>")
    .then(r => r.json())
    .then(({ data }) => {
  ...
  document.addEventListener('click', (e) => {
    ...
    const urlDett = `<?php echo e(route('ordini.dettaglio')); ?>?ordine=${encodeURIComponent(selectedOrdine)}`;
    const urlNote = `<?php echo e(route('ordini.note.list')); ?>?ordine=${encodeURIComponent(selectedOrdine)}`;
    ...
    const urlPiovan = idMes ? `<?php echo e(route('ordini.piovan')); ?>?id_mes=${encodeURIComponent(idMes)}` : null;
  ...
  document.getElementById('nota-salva')?.addEventListener('click', () => {
    ...
    fetch(`<?php echo e(route('ordini.note.save')); ?>`, {
      method: 'POST',
      headers: {
  ...
  document.addEventListener('click', (e) => {
    ...
    fetch("<?php echo e(route('ordini.piovan.lotto')); ?>", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': token,
...
<?php echo $__env->make('adminlte::page', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\guala\guala-app\resources\views/ordini/table.blade.php ENDPATH**/ ?>